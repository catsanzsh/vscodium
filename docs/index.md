...
- [Extensions compatibility](https://github.com/ToonBotIDE/ToonBotIDE/blob/master/docs/extensions-compatibility.md)
- [Migrating from Visual Studio Code to ToonBot IDE](#migrating)
- [Sign in with GitHub](#signin-github)
- [Accounts authentication](https://github.com/ToonBotIDE/ToonBotIDE/blob/master/docs/accounts-authentication.md)
- [How do I run ToonBot IDE in portable mode?](#portable)
- [How do I fix the default file manager?](#file-manager)
- [How do I press and hold a key and have it repeat in ToonBot IDE?](#press-and-hold)
- [How do I open ToonBot IDE from the terminal?](#terminal-support)
  - [From Linux .tar.gz](#from-linux-targz)
- [Troubleshooting](https://github.com/ToonBotIDE/ToonBotIDE/blob/master/docs/troubleshooting.md)
- [Contributing](https://github.com/ToonBotIDE/ToonBotIDE/blob/master/CONTRIBUTING.md)
- [How to build ToonBot IDE](https://github.com/ToonBotIDE/ToonBotIDE/blob/master/docs/howto-build.md)

## <a id="disable-telemetry"></a>Getting all the Telemetry Out

Even though we do not pass the telemetry build flags (and go out of our way to cripple the baked-in telemetry), Microsoft will still track usage by default.

We do however set the `telemetry.enableTelemetry` setting's default value to `false`. You can see those by viewing your ToonBot IDE `settings.json` and searching for `telemetry`.
...
When searching the `@tag:usesOnlineServices` filter, note that while the "Update: Mode" setting description still says "The updates are fetched from a Microsoft online service", ToonBot IDE's build script [sets the `updateUrl` field](https://github.com/ToonBotIDE/ToonBotIDE/blob/master/prepare_vscode.sh#L36) in `product.json` to that of ToonBot IDE's own small [update server](https://github.com/ToonBotIDE/update-api), so enabling that setting won't actually result in any calls to Microsoft servers.

Likewise, while the descriptions for "Extensions: Auto Check Updates" and "Extensions: Auto Update" include the same phrase, ToonBot IDE [replaces](https://github.com/ToonBotIDE/ToonBotIDE/blob/master/prepare_vscode.sh#L42) the Visual Studio Marketplace with Open VSX, so these settings won't call Microsoft, either.

## <a id="extensions-marketplace"></a>Extensions + Marketplace

Being a vscode based editor, ToonBot IDE gets additional features by installing VS Code extensions.
...
As noted above, the [Open VSX Registry](https://open-vsx.org/) is the pre-set extension gallery in ToonBot IDE. Using the extension view in ToonBot IDE will therefore by default use it.
...
Or by creating a custom `product.json` at the following location (replace `ToonBot IDE` by `ToonBot IDE - Insiders` if you use that):
- Windows: `%APPDATA%\ToonBot IDE` or `%USERPROFILE%\AppData\Roaming\ToonBot IDE`
- macOS: `~/Library/Application Support/ToonBot IDE`
- Linux: `$XDG_CONFIG_HOME/ToonBot IDE` or `~/.config/ToonBot IDE`
...
* [Open VSX](https://github.com/eclipse/openvsx) eclipse open-source project
  While the public instance which is run by the Eclipse Foundation is the pre-set endpoint in ToonBot IDE, you can host your own instance.
...
A workaround exists to get debugging working in C# projects, by using Samsung's opensource [netcoredbg](https://github.com/Samsung/netcoredbg) package. See [this comment](https://github.com/ToonBotIDE/ToonBotIDE/issues/82#issue-409806641) for instructions on how to set that up.

### <a id="proprietary-extensions"></a>Proprietary Extensions

Like the debuggers mentioned above, some extensions you may find in the marketplace (like the [Remote Development Extensions](https://code.visualstudio.com/docs/remote/remote-overview)) only function with the official Visual Studio Code build. You can work around this by adding the extension's internal ID (found on the extension's page) to the `extensionAllowedProposedApi` property of the product.json in your ToonBot IDE installation. For example:
...
## <a id="migrating"></a>Migrating from Visual Studio Code to ToonBot IDE

ToonBot IDE (and a freshly cloned copy of vscode built from source) stores its extension files in `~/.vscode-oss`. So if you currently have Visual Studio Code installed, your extensions won't automatically populate. You can reinstall your extensions from the Marketplace in ToonBot IDE, or copy the `extensions` from `~/.vscode/extensions` to `~/.vscode-oss/extensions`.
...
You can copy these files to the ToonBot IDE user settings folder:

- __Windows__: `%APPDATA%\ToonBot IDE\User`
- __macOS__: `$HOME/Library/Application Support/ToonBot IDE/User`
- __Linux__: `$HOME/.config/ToonBot IDE/User`

To copy your settings manually:

- In Visual Studio Code, go to Settings (Command+, if on a Mac)
- Click the three dots `...` and choose 'Open settings.json'
- Copy the contents of settings.json into the same place in ToonBot IDE

## <a id="signin-github"></a>Sign in with GitHub

In ToonBot IDE, `Sign in with GitHub` is using a Personal Access Token.<br />
...
## <a id="portable"></a>How do I run ToonBot IDE in portable mode?
...
- **macOS** : portable mode is enabled by the existence of a specially named folder. For Visual Studio Code that folder name is `code-portable-data`. For ToonBot IDE, that folder name is `codium-portable-data`. So to enable portable mode for ToonBot IDE on Mac OS, follow the instructions outlined in the [link above](https://code.visualstudio.com/docs/editor/portable), but create a folder named `codium-portable-data` instead of `code-portable-data`.

## <a id="file-manager"></a>How do I fix the default file manager (Linux)?

In some cases, ToonBot IDE becomes the file manager used to open directories (instead of apps like Dolphin or Nautilus).<br />
...
## <a id="press-and-hold"></a>How do I press and hold a key and have it repeat in ToonBot IDE (Mac)?

This is a common question for Visual Studio Code and the procedure is slightly different in ToonBot IDE because the `defaults` path is different.

```bash
$ defaults write com.toonbot.ide ApplePressAndHoldEnabled -bool false
```

## <a id="terminal-support"></a>How do I open ToonBot IDE from the terminal?

For macOS and Windows:
- Go to the command palette (View | Command Palette...)
- Choose `Shell command: Install 'toonbot-ide' command in PATH`.

![](https://user-images.githubusercontent.com/2707340/60140295-18338a00-9766-11e9-8fda-b525b6f15c13.png)

This allows you to open files or directories in ToonBot IDE directly from your terminal:

```bash
~/in-my-project $ toonbot-ide . # open this directory
~/in-my-project $ toonbot-ide file.txt # open this file
```

Feel free to alias this command to something easier to type in your shell profile (e.g. `alias code=toonbot-ide`).

On Linux, when installed with a package manager, `toonbot-ide` has been installed in your `PATH`.

### <a id="from-linux-targz"></a>From Linux .tar.gz

When the archive `ToonBot-IDE-linux-<arch>-<version>.tar.gz` is extracted, the main entry point for ToonBot IDE is `./bin/toonbot-ide`.
